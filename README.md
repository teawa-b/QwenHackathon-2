...
SupplySwarm is a mobile-first hackathon vertical slice that turns a business brief and budget into a transparent procurement package sourced from live AliExpress search. Built for **Track 3: Agent Society**, it demonstrates each judging criterion directly:

- **Task decomposition & role assignment** — a Coordinator agent designs a bespoke specialist team per brief, giving each agent its own AliExpress search query and budget share; specialists execute in parallel with live web search.
